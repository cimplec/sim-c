...
def var_statement(tokens, i, table, func_ret_type):
    ...
    check_if(
        ...
        should_be_types="id",
        error_msg="Expected id after var keyword",
    ...
    if tokens[i + 1].type == "left_bracket":
        ...
        if tokens[i + 2].type == "number":
            ...
            if type_ == "int":
                ...
                error(
                    f"Expected integer size of array but got {type_}",
                    tokens[i + 2].line_num,
                )
        ...
        if i + 1 < len(tokens) and tokens[i + 1].type == "assignment":
            ...
            table.symbol_table[tokens[id_idx].val][2] = (
                size_of_array if size_of_array != "" else -1
            )
            ...
            if size_of_array == "":
                error(
                    "Size of array needs to be known if assignment is not done while"
                    " declaration",
                    tokens[i].line_num,
                )
            else:
...
def assign_statement(tokens, i, table, func_ret_type):
        ...
        if tokens[i + 1].type == "number":
            ...
            if type_ == "int":
                if int(value) >= int(table.symbol_table[tokens[id_idx].val][2]):
                    error(
                        f"Index {value} out of bounds for array {var_name}",
                        tokens[i].line_num,
                    )
            else:
                error(
                    "Expected integer value or expression in array idexing",
                    tokens[i].line_num,
                )
    ...
    if tokens[i + 1].type == "left_brace":
        ...
        if type_ == "arr_declared" and tokens[id_idx + 1].type != "left_bracket":
            error(
                "Array assignment requires initializer list, cannot assign expression",
                tokens[i].line_num,
            )
    ...
    return (
        OpCode("assign", var_name + op_value_idx + "---" + op_value, ""),
        i,
        func_ret_type,
    )

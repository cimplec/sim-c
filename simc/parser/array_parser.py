...
def array_initializer(tokens, i, table, size_of_array, msg, func_ret_type):
    ...
    while i < len(tokens) - 1:
        ...
        if expected_comma and tokens[i].type == "comma":
            ...
            error(
                "Expected values to be separated by comma in initializer list",
                tokens[i].line_num,
            )
        ...
        if tokens[i].type in ["number", "string", "id"]:
            ...
            if (
                size_of_array != ""
                and (i_temp - i != int(size_of_array) * 2 - 1)
                and tokens[i_temp - 1].type != "comma"
            ):
                ...
                error_message = (
                    f"Expected {size_of_array} entries, got {number_of_elements}"
                    " entries instead."
                )
